...
class LinearNet(nn.Module):
    def __init__(self):
        ...
        self.nonlear_1 = nn.Linear(2,20)
        self.nonlear_2 = nn.Linear(20,1)
        self.second_1 = nn.Linear(2,20)
...
# forward 定义前向传播
    ...
    def forward(self,x,u0):
        ...
        for times in range(10):
            ...
            l2 = self.linear_x_1(torch.cat((encoder1,encoder2),1))
            l2 = self.Sigmoid(l2)
            l2 = self.linear_x_2(l2)
            l_second = self.second_1(torch.cat((encoder1,encoder2),1))
            l_second = self.Sigmoid(l_second)
            l_second = self.second_2(l_second)
            nonli = self.nonlear_1(torch.cat((encoder1,encoder2),1))
            nonli = self.Sigmoid(nonli)
            nonli = self.nonlear_2(nonli)
            loss1 = (1/abs(z_grad_1))*(1/abs(z_grad_2))
            #if zr.grad == 0:
            #    print('zr:',zr.grad)
            #print(zr.grad)
            ...
            loss22 = - ((encoder2 - ud2[0,1])**2) * (ud2[0, 0]) - torch.log(-ud2[0,0])/2
            loss22 = 0
            loss= loss + loss21 + torch.log(loss1) + loss22
            #loss2_sum = loss2_sum - ((encoder - ud1[0, 1]) ** 2) * (ud1[0, 0]) - torch.log(-ud1[0, 0]) / 2
            #print(loss1)
                #loss.backward(retain_graph=True)
                #print(self.params1.grad)
            #encoder1_test = torch.rand(1,1,out=None)*10 - 5
            #encoder2_test = torch.rand(1, 1, out=None)*10 - 5
            encoder1_test = encoder1
            encoder2_test = encoder2
            l2_test = self.linear_x_1(torch.cat((encoder1_test,encoder2_test),1))
            l2_test = self.Sigmoid(l2_test)
            l2_test = self.linear_x_2(l2_test)
            ge_z1.append(encoder1_test.cpu())
            ge_z2.append(encoder2_test.cpu())
            ge_x.append(l2_test.cpu())
        ...
        return loss/10,ge_z1,ge_x,ge_z2,loss2_sum/100
...
def train():
    ...
    print('tl:',net.params1)
    net.params2.data = net.params2.data - 0.0001 * net.params2.grad
