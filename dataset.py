...
# forward 定义前向传播
    ...
    def forward(self,x,u0):
        ...
        for times in range(50):
            ...
            loss21 = - ((encoder1 - ud1[0,1])**2) * (ud1[0, 0]) - torch.log(-ud1[0,0])/2 + torch.norm((l2 - x),p=2)*20000 + (encoder1**2 + encoder2**2 - 1)
...
def train():
    ...
    x0 = x0_list[0:10]
    x0 = torch.tensor(x0).view(10,1)*5
    x1 = x1_list[0:10]
    x1 = torch.tensor(x1).view(10,1)*5
    x2 = x2_list[0:10]
    x2 = torch.tensor(x2).view(10,1)*5
    x3 = x3_list[0:10]
    x3 = torch.tensor(x3).view(10, 1)*5
    ...
    x_total = torch.cat((x_total,x_new2),1) + (torch.randn(30,4)/100)
    u = torch.cat((u0, u1), 0)
    u = torch.cat((u,u2),0)
    #u = torch.cat((u,u3),0)
    ...
    net.params1.data = net.params1.data - 0.01 * net.params1.grad
    print('tl:',net.params1)
