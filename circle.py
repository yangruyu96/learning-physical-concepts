...
# forward 定义前向传播
    ...
    def forward(self,x,u0):
        ...
        for times in range(100):
            ...
            encoder2 = self.encoder2_1(encoder_01)
            ...
            l2 = self.linear_x_1(torch.cat((encoder1_test, encoder2_test), 1))
            ...
            loss21 = - ((encoder1 - ud1[0,1])**2) * (ud1[0, 0]) - torch.log(-ud1[0,0])/2 + torch.norm((l2 - x),p=2)*20000 + (encoder1**2 + encoder2**2 - 1) * 100
            loss= loss + loss21 + torch.log(loss1)
            loss2_sum = loss2_sum + torch.norm((l2 - x),p=2)
...
def train():
    ...
    for time in range(2000000):
        # print(time)
        gene = generate(4, Q, TL, 3)
        if gene[1] > 0:
            x3_list.append(gene[0])
            y3_list.append(gene[1])
    x0 = x0_list[0:20]
    x0 = torch.tensor(x0).view(20,1)*5
    x1 = x1_list[0:20]
    x1 = torch.tensor(x1).view(20,1)*5
    x2 = x2_list[0:20]
    x2 = torch.tensor(x2).view(20,1)*5
    x3 = x3_list[0:20]
    x3 = torch.tensor(x3).view(20, 1)*5
    ...
    x_new3 = np.sin(x)
    x_new1 = np.cos(x)
    #plt.plot(x_new1, x_new3, '.')
    #plt.show()
    x_new2 = np.sin(x+0.1)
    x_new4 = np.cos(x+0.1)
    x_total = torch.cat((x_new3,x_new1),1)
    x_total = torch.cat((x_total,x_new4),1)
    x_total = torch.cat((x_total,x_new2),1) + (torch.randn(60,4)/100)
    u = torch.cat((u0, u1), 0)
    u = torch.cat((u,u2),0)
    #u = torch.cat((u,u3),0)
    mydataset = Data.TensorDataset(x_total, u)
    ...
    net.params1.data = net.params1.data - 0.0001 * net.params1.grad
    print('tl:',net.params1)
...
for epoch in range(10000):
    ...
    if epoch%300 == 1:
        ...
        #my3d.plot(ge_x_1, ge_1_1, ge_1_2, '.')
        #my3d.plot(ge_x_2, ge_2_1, ge_2_2, '.')
        #my3d.plot(ge_x_3, ge_3_1, ge_3_2, '.')
        #plt.xlabel('measure')
        plt.plot(ge_1_1,ge_1_2)
        plt.plot(ge_2_1,ge_2_2)
        plt.plot(ge_3_1,ge_3_2)
