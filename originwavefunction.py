...
# forward 定义前向传播
    ...
    def forward(self,x,u0):
        ...
        for times in range(100):
            sample0 = torch.randn(1,1)*torch.sqrt(abs(encoder[0,0])) + encoder[0,3]
            sample1 = torch.randn(1,1)*torch.sqrt(abs(encoder[0,1])) + encoder[0,4]
            sample2 = torch.randn(1,1)*torch.sqrt(abs(encoder[0,2])) + encoder[0,5]
            sample = torch.tensor([sample0,sample1,sample2])
            l2 = self.linear_x_1(sample)
            l2 = self.Sigmoid_T(l2)
            l2 = self.linear_x_2(l2)
            l2 = self.Sigmoid_T(l2)
            l2 = self.linear_x_3(l2)
            ge_z1.append(sample0.cpu().detach().numpy())
            ge_z2.append(sample1.cpu().detach().numpy())
            ge_x.append(sample2.cpu().detach().numpy())
            loss2_sum = loss2_sum + (torch.norm((l2 - x),p=2)**2)*10
            loss_re = loss_re + fun1(sample[0], sample[1], sample[2]) * 100000
        ...
        divergence1 = (-1 / 2) * (math.log(2 * math.pi) + math.log(abs(encoder[0, 0] * encoder[0, 1] * encoder[0, 2])))
        loss21 = loss2_sum/100
        loss = loss21 + loss_re / 100 + divergence1
        #loss = loss21 + (1/2)*(loss_kl_1 + loss_kl_2 + loss_kl_final) + loss_re/100
        #print('loss_kl_1:',loss_kl_1,'loss_kl_2:',loss_kl_2,'loss_kl_final:',loss_kl_final,'loss21:',loss21/100,'loss_re:',loss_re/100)
...
def train():
    ...
    for k in range(0,100):
        ...
        y0.append([math.cos(x1),math.sin(x1)*math.cos(x2),math.sin(x1)*math.sin(x2),math.cos(x1)+math.sin(x1)*math.cos(x2),math.cos(x1)+math.sin(x1)*math.sin(x2)])
            #y0.append(pro(a0_list[k],b0_list[k],x/15))
        y_total.append(y0)


    y_total = torch.tensor(y_total).view(100,5) + (torch.randn(100,5)/100)


    u0 = torch.autograd.Variable(torch.ones(100, 1), requires_grad=True)

    mydataset = Data.TensorDataset(y_total, u0)
...
for epoch in range(10000):
    ...
    if epoch == 20:
        ...
        plt.plot(xarr,np.log(loss_model))
        plt.show()
    #print(t[0])
