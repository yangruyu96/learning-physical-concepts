...
# forward 定义前向传播
    ...
    def forward(self,x,u0):
        ...
        for times in range(50):
            ...
            loss21 = - ((encoder1 - ud1[0,1])**2) * (ud1[0, 0])/2 - torch.log(-ud1[0,0])/2 + (torch.norm((l2 - x),p=2)**2)*20000 #+ (encoder1**2 + encoder2**2 - 1) * 100
...
def wave(a,b,x):
    return (a * base(x,1) + b * base(x,2))/(abs(a)**2 + abs(b)**2)

def pro(a,b,x):
    return (abs(wave(a,b,x))**2)/1000
...
def train():
    ...
    y_total = torch.tensor(y_total).view(60,10) + (torch.randn(60,10)/100)
