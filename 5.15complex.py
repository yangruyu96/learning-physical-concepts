...
# forward 定义前向传播
    ...
    def forward(self,x,u0):
     ...
     #       print('sample:',sample,'r:',r)
            ...
            loss_re = loss_re + torch.abs(torch.norm((torch.randn(1, 1) * torch.sqrt(torch.abs(encoder[0, 0])) + encoder[0, 3]),p=2)**2 + torch.norm((torch.randn(1, 1) * torch.sqrt(torch.abs(encoder[0, 1])) + encoder[0, 4]),p=2)**2 + torch.norm((torch.randn(1, 1) * torch.sqrt(torch.abs(encoder[0, 2])) + encoder[0, 5]),p=2)**2 - 1) * 1000000
...
#        loss_re = torch.tensor(loss_re,requires_grad=True)
        loss = loss21 + divergence1 + loss_re/100
