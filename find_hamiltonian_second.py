...
class LinearNet_hamiltonian(nn.Module):
    def __init__(self):
        ...
        self.params1 = nn.Parameter(torch.tensor([[1,0],[1,0],[1,0]]),requires_grad=False)
        self.encoder1_1 = nn.Linear(6,30)
        self.encoder1_2 = nn.Linear(30,30)
        self.encoder1_3 = nn.Linear(30,30)
        self.encoder1_4 = nn.Linear(30,4)
        self.Sigmoid = nn.ReLU()
        self.Sigmoid_T = nn.Tanh()
        self.linear_x_1 = nn.Linear(4,30)
        self.linear_x_2 = nn.Linear(30,30)
        self.linear_x_3 = nn.Linear(30,30)
        self.linear_x_4 = nn.Linear(30,6)
# forward 定义前向传播
    def decoder_1(self,z,n):
        n = n.cpu().detach().numpy() - 1
        u = self.params1[n]
    ...
    def forward(self,x,u0):
        ...
        encoder = self.encoder1_3(encoder)
        encoder = self.Sigmoid(encoder)
        encoder = self.encoder1_4(encoder)
        x1 = encoder[0,0].cpu().detach().numpy()
        x2 = encoder[0,1].cpu().detach().numpy()
        x3 = encoder[0, 2].cpu().detach().numpy()
        radius = np.sqrt(x1**2 + x2**2 + x3 **2)
        ...
        ge_x.append(x3)
        loss21 = loss2_sum/100
        loss =  loss2_sum + loss_re + (encoder[0,3]**2)*10000
        print('loss_re:',loss_re)
        print('loss2_sum:',loss2_sum)
        print('topo:',(encoder[0,3]**2))
        return loss,ge_z1,ge_x,ge_z2,loss2_sum/50
#net_state = LinearNet()
net_state = torch.load('net_more_latent')

net_hamiltonian = LinearNet_hamiltonian()
#net_hamiltonian = torch.load('6.31')
...
def generate_hamiltonian_Data():
    ...
    for k in range(0,1000):
        ...
        for times in range(0,2):
            ...
            output = torch.tensor(hanshuku.tomography(rho), dtype=torch.float32)
            time_series.extend([output[0,1],output[0,2],output[0,3]])
            x.append(output[0,1].cpu().detach().numpy())
            y.append(output[0,2].cpu().detach().numpy())
            z.append(output[0,3].cpu().detach().numpy())
        #print(time_series)
...
def train_hamiltonian():
    ...
    #print(y_total)
    y_total = torch.tensor(y_total).view(100, 6) #+ (torch.randn(100, 30) / 10000)
    ...
    optimizer.step()
    optimizer.zero_grad()
    torch.save(net_hamiltonian,'7.2')
