...
# forward 定义前向传播
    ...
    def forward(self,x,u0):
        ...
        for times in range(100):
            ...
            #r = math.sqrt(sample0**2 + sample1**2 + sample2**2)
     #       print('sample:',sample,'r:',r)
            #sample = sample/r
            l2 = self.linear_x_1(torch.tensor([torch.randn(1, 1) * torch.sqrt(torch.abs(encoder[0, 0])) + encoder[0, 3],torch.randn(1, 1) * torch.sqrt(torch.abs(encoder[0, 1])) + encoder[0, 4],torch.randn(1, 1) * torch.sqrt(torch.abs(encoder[0, 2])) + encoder[0, 5]]))
            l2 = self.Sigmoid(l2)
            l2 = self.linear_x_2(l2)
            l2 = self.Sigmoid(l2)
            l2 = self.linear_x_3(l2)
            iden = torch.tensor([1,1,1])
            loss2_sum = loss2_sum + (torch.norm((l2 - x),p=2)**2) * 10000
            loss_re = loss_re + torch.abs(torch.norm((torch.randn(1, 1) * torch.sqrt(torch.abs(encoder[0, 0])) + encoder[0, 3]),p=2)**2 + torch.norm((torch.randn(1, 1) * torch.sqrt(torch.abs(encoder[0, 1])) + encoder[0, 4]),p=2)**2 + torch.norm((torch.randn(1, 1) * torch.sqrt(torch.abs(encoder[0, 2])) + encoder[0, 5]),p=2)**2 - 1) * 10000
            ...
            ge_z1.append(sample[0].cpu().detach().numpy())
            ge_z2.append(sample[1].cpu().detach().numpy())
            ge_x.append(sample[2].cpu().detach().numpy())
...
#        loss_re = torch.tensor(loss_re,requires_grad=True)
        loss = loss21 + loss_kl_2 + loss_kl_1 + loss_kl_final + loss_re/100
        print('loss21:',loss21,'lossre:',loss_re/100)
...
def train():
    ...
    y_total = torch.tensor(y_total).view(100,5) + (torch.randn(100,5)/10000)
